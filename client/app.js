(function(){
    ...
    Template.body.events({
        ...
        'click #saveNote': function(event){
            var config = Session.get('chosenConfig'),
                dateStr = new Date().toLocaleString(),
                prevVal = Config.findOne(config).notes || "",
                note = prevVal + dateStr + "<br/>" + $("#comment").val() + "<br/> -------------------------------------<br/>";
            Config.update({_id:config._id}, {$set: {notes:note}});
            configLoad(config.ConfigName);
        }
    });
})();
